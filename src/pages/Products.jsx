...
import { FaAngleLeft, FaAngleRight, FaFilter, FaTimes, FaThLarge, FaList, FaFire, FaClock } from "react-icons/fa";
  ...
  const renderPagination = () => {
    ...
    return pages.map((p, idx) => {
      if (idx > 0 && p - pages[idx - 1] > 1) {
        return (
          <span key={`ellipsis-${p}`} className="px-2 text-slate-300">
            ···
          </span>
        );
      }
      ...
      return (
        <button
          ...
          className={`w-9 h-9 rounded-lg font-600 text-sm transition-all ${
            page === p
              ? "bg-gradient-to-br from-indigo-500 to-blue-500 text-white shadow-lg shadow-indigo-400/40"
              : "bg-white border-1.5 border-indigo-200 text-indigo-600 hover:bg-indigo-50 hover:border-indigo-300"
          }`}
        >
          {p}
        </button>
  ...
  return (
    <>
      <style>{`
        ...
        .prod-root * { font-family:'Plus Jakarta Sans',sans-serif; }
        .prod-serif { font-family:'Playfair Display',serif; }
        ...
        .prod-header { animation: prodFadeDown 0.6s cubic-bezier(0.22,1,0.36,1); }
        .prod-pagination { animation: prodSlideUp 0.5s cubic-bezier(0.22,1,0.36,1); }
        .prod-float-btn { animation: prodBounce 2s ease-in-out infinite; }
        .prod-float-btn:hover { animation: none; transform: scale(1.12); }
        .prod-toolbar { animation: prodSlideInLeft 0.4s cubic-bezier(0.22,1,0.36,1); }
        ...
        .prod-empty {
          ...
          align-items:center;
          min-height:400px;
          animation:prodFadeDown 0.5s cubic-bezier(0.22,1,0.36,1);
        }
        ...
        .prod-grid {
          ...
          gap:12px;
          animation:prodFadeDown 0.4s cubic-bezier(0.22,1,0.36,1) 0.1s backwards;
        }
        ...
        .prod-float-btn:active { transform:scale(0.95); }

        .prod-skeleton-grid {
          display:grid;
          grid-template-columns:repeat(auto-fill, minmax(160px, 1fr));
          gap:12px;
          animation:prodFadeDown 0.3s cubic-bezier(0.22,1,0.36,1);
        }
        @media(min-width:640px) {
          .prod-skeleton-grid {
            grid-template-columns:repeat(auto-fill, minmax(180px, 1fr));
        ...
        .prod-page-btn:active:not(:disabled) {
        ...
        .filter-badge:hover {
          background:linear-gradient(135deg,rgba(99,102,241,0.18),rgba(37,99,235,0.12));
        ...
        .sort-item:hover { background:#f8faff; color:#4f46e5; font-weight:600; }
        .sort-item.active { background:linear-gradient(135deg,rgba(99,102,241,0.1),rgba(37,99,235,0.08)); color:#4f46e5; font-weight:700; }
      ...
      <div className="prod-root min-h-screen bg-gradient-to-br from-slate-50 via-indigo-50/40 to-blue-50/30 py-8 px-3 sm:px-4 lg:px-6">
        ...
        <div className="fixed inset-0 pointer-events-none overflow-hidden z-0">
          ...
          <div className="absolute top-1/3 -right-32 w-72 h-72 bg-blue-300/10 blur-3xl rounded-full animate-pulse" style={{ animationDelay: "1s" }} />
        ...
        <div className="relative z-10">
          ...
          {/* Header */}
          <div data-aos="fade-down" className="prod-header mb-8 text-center">
            <div className="inline-flex items-center gap-3 mb-4 px-5 py-2 rounded-full bg-white/70 backdrop-blur border border-indigo-100 shadow-sm">
              <MdOutlineCategory className="text-indigo-500" size={18} />
            ...
            <p className="text-slate-500 text-sm sm:text-base max-w-2xl mx-auto">
              Discover premium products curated just for you. Filter by category, brand, and price to find exactly what you need.
            </p>
          </div>

          {/* Toolbar */}
          {!loading && filteredProducts.length > 0 && (
            <div data-aos="fade-up" className="prod-toolbar toolbar-card">
              ...
              <div className="toolbar-right">
                    ...
                    <div className="sort-menu">
                      {[
                        ...
                        <div
                          ...
                          className={`sort-item ${sort === item.value ? "active" : ""}`}
                        >
                          {sort === item.value && "✓"}
                          {item.label}
                        </div>
                ...
                <div className="view-toggle">
                  <button
                    ...
                    onClick={() => { setGridView(true); setPage(1); }}
                    title="Grid view"
                  >
                    <FaThLarge size={14} />
                  </button>
                  <button
                    className={`view-btn ${!gridView ? "active" : ""}`}
                    onClick={() => { setGridView(false); setPage(1); }}
                    title="List view"
                  >
                    <FaList size={14} />
                  </button>
          ...
          {loading ? (
            ...
            <div data-aos="zoom-in" className="prod-empty">
            ...
            <>
              ...
              {/* Pagination */}
              {totalPages > 1 && (
                <div data-aos="fade-up" className="prod-pagination prod-pagination-wrap">
                  <button
                    onClick={() => { setPage((prev) => Math.max(prev - 1, 1)); window.scrollTo({ top: 0, behavior: "smooth" }); }}
                    disabled={page === 1}
                    className="prod-page-btn w-10 h-10 rounded-lg border-1.5 border-indigo-200 bg-white text-indigo-600 hover:bg-indigo-50 hover:border-indigo-300 flex items-center justify-center transition-all font-600"
              ...
              {/* Results Info */}
              <div data-aos="fade-up" className="text-center mt-8 text-sm text-slate-500">
                Showing{" "}
